...
final_data <- final_data %>%
  mutate(
    PortfolioEU_speakerinfo = as.factor(PortfolioEU_speakerinfo),  # Convert portfolio to categorical
...
logit_glmm_1 <- glmer(claimbinary ~ PortfolioEU_speakerinfo + 
...
# Add Length
logit_glmm_2 <- glmer(claimbinary ~ PortfolioEU_speakerinfo + length + 
                        (1 | Commission_speakerinfo),  
                      data = final_data, 
                      family = binomial,
                      control = glmerControl(optimizer = "bobyqa"))
...
# Add w_top_tot
logit_glmm_3 <- glmer(claimbinary ~ PortfolioEU_speakerinfo + length + w_top_tot +
                        (1 | Commission_speakerinfo),  
                      data = final_data, 
                      family = binomial,
                      control = glmerControl(optimizer = "bobyqa"))
...
# Add w_top_rel
logit_glmm_4 <- glmer(claimbinary ~ PortfolioEU_speakerinfo + length + European.Party_speakerinfo +
                        (1 | Commission_speakerinfo),  
                      data = final_data, 
                      family = binomial,
                      control = glmerControl(optimizer = "bobyqa"))
...
logit_glmm_5 <- glmer(claimbinary ~ PortfolioEU_speakerinfo * Sex_speakerinfo + length + European.Party_speakerinfo +
                        (1 | Commission_speakerinfo),  
                      data = final_data, 
                      family = binomial,
                      control = glmerControl(optimizer = "bobyqa"))

logit_glmm_6 <- glmer(claimbinary ~ PortfolioEU_speakerinfo * Sex_speakerinfo + length + European.Party_speakerinfo + w_top_tot +
                        (1 | Commission_speakerinfo),  
                      data = final_data, 
                      family = binomial,
                      control = glmerControl(optimizer = "bobyqa"))
...
predicted_df <- cbind(final_data[, c("PortfolioEU_speakerinfo", "Sex_speakerinfo")], predicted_df)

# Convert factor levels
predicted_df$PortfolioEU_speakerinfo <- factor(predicted_df$PortfolioEU_speakerinfo, 
                                              levels = c(1, 2, 3), 
                                              labels = c("Masculine", "Neutral", "Feminine"))
...
# Create the scatter plot with thicker error bars and a black frame
plot2 <- ggplot(predicted_df, aes(x = PortfolioEU_speakerinfo, y = predicted, color = Sex_speakerinfo)) +
  geom_point(size = 4, position = position_dodge(width = 0.3)) +  # Scatter points, slightly dodged
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.2, size = 1.3,  # Thicker error bars
                position = position_dodge(width = 0.3), alpha = 0.8) +
  scale_color_manual(values = c("Men" = "#003399", "Women" = "#FFCC00")) +  # Custom colors
...
# Save the plote
ggsave(filename = paste0(save_folder, "Interaction_Effect_Portfolio_Sex_CountEU.svg"), 
...
predicted_df <- cbind(final_data[, c("PortfolioEU_speakerinfo", "Sex_speakerinfo", "Commission_speakerinfo")], predicted_df)
...
# Convert PortfolioEU_speakerinfo and Sex_speakerinfo to factors
predicted_df$PortfolioEU_speakerinfo <- factor(predicted_df$PortfolioEU_speakerinfo, 
                                              levels = c(1, 2, 3), 
                                              labels = c("Masculine", "Neutral", "Feminine"))
...
plot2 <- ggplot(predicted_df, aes(x = Commission_speakerinfo, y = predicted, 
                                  color = PortfolioEU_speakerinfo, group = PortfolioEU_speakerinfo)) +
  geom_line(size = 1.5) +  # Line plot for trends
...
# Save the plot
ggsave(filename = paste0(save_folder, "Predicted_Claims_by_Portfolio_Over_TimeEU.svg"), 
...
predicted_avg <- predicted_df %>%
  group_by(Commission_speakerinfo, Sex_speakerinfo, PortfolioEU_speakerinfo) %>%
  summarise(
...
for (category in c("Masculine", "Neutral", "Feminine")) {
  ...
  plot <- ggplot(predicted_avg %>% filter(PortfolioEU_speakerinfo == category), 
  ...
  ggsave(filename = paste0(save_folder, "Predicted_Claims_", category, "_Portfolio_Over_TimeEU.svg"), 
...
for (category in c("Masculine", "Neutral", "Feminine")) {
  ...
  ggsave(filename = paste0(save_folder, "Predicted_Claims_", category, "_Portfolio_Marginal_EffectsEU.svg"), 
         plot = plot, width = 10, height = 6, dpi = 300, device = "svg")
}
